...
#define CT_HTONS(n) htons(n)
#define CT_NTOHS(n) ntohs(n)
#define CT_HTONL(n) htonl(n)
...
int obfs_app_data(buffer_t *buf, size_t cap, obfs_t *obfs)
{
...
int deobfs_app_data(buffer_t *buf, size_t idx, obfs_t *obfs)
{
...
int obfs_tls_request(buffer_t *buf, size_t cap, obfs_t *obfs, obfs_para_t *obfs_para)
{
    ...
    if (obfs->obfs_stage == 0) {
        ...
        size_t host_len = strlen(obfs_para->host);
        ...
        memcpy((char *)server_name + server_name_len, obfs_para->host, host_len);
...
int deobfs_tls_response(buffer_t *buf, size_t cap, obfs_t *obfs)
{
